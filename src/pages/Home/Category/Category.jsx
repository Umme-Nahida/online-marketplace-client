...
const Category = () => {
  ...
  const categoryOne = allCategoryJobs?.filter(
    (job) => job?.category === "web-development"
  );
  const categoryTwo = allCategoryJobs?.filter(
    (job) => job?.category === "digital-marketing"
  );
  const categoryThree = allCategoryJobs?.filter(
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5 md:gap-10 m-10 md:m-20 justify-items-center ">
            {categoryOne.map((job) => (
              <div key={job?._id} className="card  bg-base-100 shadow-xl">
                <div className="min-w-[550px] min-h-[250px] flex items-center justify-start p-10">
                  <div className="space-y-3">
                    ...
                    <div className="card-actions justify-start">
                      <Link to={`/jobDetails/${job._id}`}>
                        <button className="btn bg-[#2071AB] text-white hover:text-black">
                          Bid Now
                        </button>
                      </Link>
                    </div>
